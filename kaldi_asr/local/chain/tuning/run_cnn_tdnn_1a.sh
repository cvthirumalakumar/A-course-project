...
graph_dir=$dir/graph_lmCombinedSmall
if [ $stage -le 16 ]; then
  # Note: it might appear that this $lang directory is mismatched, and it is as
  # far as the 'topo' is concerned, but this script doesn't read the 'topo' from
  # the lang directory.
  utils/mkgraph.sh --self-loop-scale 1.0 --remove-oov data/lang_test_lmCombinedSmall $dir $graph_dir
fi
...
if [ $stage -le 17 ]; then
  ...
  for decode_set in test_msr; do
      (
      steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
          --nj $decode_nj --cmd "$decode_cmd" $iter_opts \
          --online-ivector-dir exp/nnet3${nnet3_affix}/ivectors_${decode_set}_hires \
          $graph_dir data/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_lmCombinedSmall || exit 1
      # steps/lmrescore.sh --cmd "$decode_cmd" --self-loop-scale 1.0 data/lang_test_{lmCombinedSmall,lmCombinedLarge} \
      #     data/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{lmCombinedSmall,lmCombinedLarge} || exit 1
      steps/lmrescore_const_arpa.sh \
          --cmd "$decode_cmd" data/lang_test_{lmCombinedSmall,lmLarge} \
          data/${decode_set}_hires $dir/decode_${decode_set}${decode_iter:+_$decode_iter}_{lmCombinedSmall,lmCombinedLarge} || exit 1
      ) || touch $dir/.error &
  done
  wait
  ...
  for data in test_msr; do
    (
      ...
      steps/online/nnet3/decode.sh \
          ...
          $graph_dir data/${data} ${dir}_online/decode_${data}_lmCombinedSmall || exit 1
