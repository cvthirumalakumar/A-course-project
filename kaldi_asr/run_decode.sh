...
test_sets="test_vits test_tacatron"
dumpdir=dump
data_folder=data
mfccdir="$dumpdir/mfcc_$data_folder"
lmtags="tgsmall bea_3"
expdir=exp
...
for datadir in $test_sets; do 
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 1 \
        ${data_folder}/${datadir}_hires $expdir/nnet3_cleaned/extractor \
        $expdir/nnet3_cleaned/ivectors_${datadir}_hires || exit 1;
done
...
for lmtype in $lmtags;do
    ...
    for decode_set in $test_sets; do
        steps/nnet3/decode.sh --use-gpu false --acwt 1.0 --post-decode-acwt 10.0 --nj 1 --num-threads 4 \
            ...
            $expdir/nnet3_cleaned/ivectors_${decode_set}_hires $graph_dir \
